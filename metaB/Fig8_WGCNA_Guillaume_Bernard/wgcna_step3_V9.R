...
#workingDir = args[1];
workingDir = "/Users/6ui11aum3/Documents/ibens/juan_v9_diatoms/V9/wgcna_diatoms_V9_filtered_pooled"
setwd(workingDir); 
...
lnames = load(file = "diatoms_pfams_networkConstruction-pow20_merge0.75minMod40.RData")
lnames
...
#moduleTraitPvalue2 <- p.adjust(moduleTraitPvalue, method="fdr")

moduleTraitPvalue2 <- matrix(p.adjust(as.vector(as.matrix(moduleTraitPvalue)), method='fdr'),ncol=50)
...
write.csv(signif(moduleTraitCor, 2), "V9_module_envVariable_correlations_matrix.csv")
write.csv(signif(moduleTraitPvalue2, 1), "V9_module_envVariable_pvalues_matrix.csv")
...
dir_output = "gene_MM_GS_info_pow4_merge0.25"
...
write.tree(phy=my_tree, file="V9_module_clustering_tree.newick")
...
rownames(combine_info) = rownames(datExpr)
...
combine_info <- cbind(combine_info, sample = rownames(datExpr))
...
write.csv(pfam_module,"ASV_module_pow20_merge0.75_minMod40.csv",row.names = FALSE)
